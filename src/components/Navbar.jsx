...
const Navbar = ({ activeRole, onRoleChange, isDark = false }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 w-full py-4 z-50 transition-all duration-500 ${isScrolled
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between relative">
        {/* Logo */}
        <a href="#" className={`flex items-center gap-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:rounded-lg ${isDark ? 'focus-visible:ring-[#3F8F6B]' : 'focus-visible:ring-primary'
          }`}>
          <img
            src={logo.src}
            alt={logo.alt}
          ...
          />
          <span className={`text-xl font-bold transition-colors duration-500 ${isDark && !isScrolled ? 'text-white' : 'text-gray-900'
            }`}>Pillo</span>
        </a>
